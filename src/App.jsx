...
export default function App(){
  ...
  const [finTab,setFinTab]=useState("fixas");
  // ── Metas ──
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"0 20px"}}>
        ...
        <div style={{marginBottom:40}}>
          ...
          <div style={{display:"flex",gap:8,borderBottom:`1px solid ${BDR}`,marginBottom:20,overflowX:"auto",paddingBottom:12}}>
            {[{id:"fixas",label:"Despesas Fixas"},{id:"receitas",label:"Receitas"},{id:"variaveis",label:"Variáveis"},{id:"contas",label:"Contas"}].map(t=>(
              <button key={t.id} onClick={()=>setFinTab(t.id)} style={{padding:"8px 16px",borderRadius:8,border:"none",background:finTab===t.id?"rgba(255,255,255,0.1)":"transparent",color:"#e5e5e5",fontSize:12,fontWeight:finTab===t.id?700:500,cursor:"pointer",whiteSpace:"nowrap"}}>{t.label}</button>
            ))}
          </div>
